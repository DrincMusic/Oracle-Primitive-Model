...
document_control:
  ...
  scientific_core_authorized: true
  intended_next_state: IMPLEMENTATION_VALIDATION
  normative_keywords: [must, must_not, should, may, exploratory]
```

**Authority:** This document defines the owner-approved, directly implementable OPM v1 experiment under the constraints of `ORACLE_PRIMITIVE_MODEL.md`. Version 1.0.0 authorizes scientific-core translation and `IMPLEMENTATION_VALIDATION`; it does not authorize primary experiments before protocol freeze.
...
approval:
  ...
  specification_version: 1.0.0
  approved_commit_or_hash: normative-prefix-sha256:d29323d511e6b9813a2e874b71e78b2836b2000cebf0d2649abcbe284b626ffe
  candidate_normative_prefix_sha256: d29323d511e6b9813a2e874b71e78b2836b2000cebf0d2649abcbe284b626ffe
  effective_post_transition_normative_prefix_sha256: 7f986ec9d979fa37f180d57e5d8cf5d9830eec35901d9d6c57fdd24819d57713
  approved_by: Dwight Robert Keller-Williams
  approval_date: 2026-08-20
  approval_statement: I, Dwight Robert Keller-Williams approve OPM v1 Implementation Specification version 1.0.0 with normative-prefix SHA-256 d29323d511e6b9813a2e874b71e78b2836b2000cebf0d2649abcbe284b626ffe.
  approved_scope: OPM v1 scientific-core translation and IMPLEMENTATION_VALIDATION
  known_exceptions:
    - PyTorch and target accelerator environment remain to be resolved during IMPLEMENTATION_VALIDATION
  next_lifecycle_state: IMPLEMENTATION_VALIDATION
```

The scientific owner explicitly approved version 1.0.0 and its candidate normative digest. Scientific-core translation and `IMPLEMENTATION_VALIDATION` are authorized. Primary experiments remain prohibited until implementation conformance, environment validation, raw leakage gates, dataset sealing, and `PROTOCOL_FROZEN` entry criteria pass.

The approval-basis digest is SHA-256 over the exact UTF-8 bytes from the beginning of the approval candidate up to, but excluding, the `## 17. Approval record` heading. The owner's approval mechanically changes lifecycle metadata within that prefix. Therefore, the audit record preserves the exact owner-approved candidate digest and separately records the effective post-transition prefix digest. No scientific requirement changes during this transition.
...
- [x] Approval record is filled by the scientific owner.

The remaining implementation-validation items block `PROTOCOL_FROZEN` and primary runs.
